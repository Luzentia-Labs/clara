...
| A placeholder reached by a screen reader | Nothing. It is `aria-hidden="true"` and the props expose no way to change that - no `children`, no `role`, no `aria-*` |
...
| --- | --- | --- | --- |
| AC1 | packages/react/src/components/Skeleton/Skeleton.tsx | TWO mutants, both KILLED, one per half of the split. (a) Drop `aria-hidden="true"` from the placeholder - 2 tests fail, and a forty-placeholder table skeleton becomes forty announcements. (b) Delete the group's `<span className="clara-visually-hidden">{label}</span>` - 2 fail, and the loading state is announced zero times instead of once. The criterion is "announces exactly once", so both directions have to redden. | Hidden from AT |

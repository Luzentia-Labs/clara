...
CheckboxGroup) are on `main`, with 917 tests and every gate green. Nothing is on npm. The stories
...
- `pnpm check` runs **28 guards**; `prove-guards-fail` kills **103 mutations** on a staged copy.
- **917 tests.** **19 CI gates**, 18 wired; the one pending is gate 7 (visual regression), owned by

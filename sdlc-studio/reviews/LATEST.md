...
The tree is on `main` with 1247 tests and every gate green. Nothing is on npm.

## Numbers

- `pnpm check` runs **30 guards**; `prove-guards-fail` kills **147 mutations** on a staged copy.
- **1247 tests.** **19 CI gates**, 18 wired; the one pending is gate 7 (visual regression), owned by

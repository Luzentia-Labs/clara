...
CheckboxGroup) are on `main`, with 1022 tests and every gate green. Nothing is on npm. The stories
...
- `pnpm check` runs **29 guards**; `prove-guards-fail` kills **128 mutations** on a staged copy.
- **1022 tests.** **19 CI gates**, 18 wired; the one pending is gate 7 (visual regression), owned by

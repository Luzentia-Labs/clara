...
Horizontal layout that wraps, with token-scaled gaps.
...
Wrapping is the default rather than an overflow, so a long row reflows instead of being clipped at narrow widths or under a large text-size preference.

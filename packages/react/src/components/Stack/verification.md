...
Vertical layout with token-scaled gaps.
...
Spacing is `gap` on a flex container, never margins on children. Collapsing and doubling margins are the two ways spacing silently changes when a child is swapped, and `gap` has neither.
